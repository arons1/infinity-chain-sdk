...
export const encode = ({
    ...
    if(transaction["tx"]["TransactionType"] != "Payment" && transaction["tx"]["TransactionType"] != "AccountDelete" && transaction["tx"]["TransactionType"] != "TrustSet")
      return;
    var value = transaction["tx"]["Amount"]
    var from = transaction["tx"]["Account"];
    var to = transaction["tx"]["Destination"];
    var fee = transaction["tx"]["Fee"]
    ...
    var confirmations = "7";
    if (transaction["validated"] != undefined && transaction["validated"] == false)
        confirmations = "0";
    if (from == to || transaction["meta"]["TransactionResult"] != "tesSUCCESS") {
        value = "0"
    }
    return {
        blockNumber:transaction.tx.ledger_index,
        timeStamp: new Date(transaction["tx"]["date"]+946684800).toISOString(),
