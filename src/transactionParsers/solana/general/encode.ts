...
export const encode = ({
    ...
    var balanceBefore = accountIndex == -1 ? 0 : transaction.details.meta.preBalances[accountIndex]
    var balanceAfter = accountIndex == -1 ? 0 : transaction.details.meta.postBalances[accountIndex]
    var blockheight = transaction.details.slot
    var time = transaction.details.blockTime*1000
    const filteredPreTokens = transaction.details.meta.preTokenBalances.filter(a => a.owner == account)
    const postBalancesAux = transaction.details.meta.postTokenBalances.filter(a => a.owner == account)
    const notIn = postBalancesAux.filter(a => filteredPreTokens.find(b => b.mint == a.mint) == undefined)
    ...
    for(let preBalance of filteredPreTokens){
        const postBalance = transaction.details.meta.postTokenBalances.filter(a => a.owner == account && a.mint == preBalance.mint)[0]
        if(postBalance == undefined)
            continue;
        var value = new BigNumber(postBalance.uiTokenAmount.amount).minus(preBalance.uiTokenAmount.amount).toNumber()
        var outToken = 0;
        if(value < 0){
            outToken = 1
            value *= -1
        ...
        var otherSender = transaction.details.meta.postTokenBalances.filter(a => a.owner != account && a.mint == preBalance.mint)[0]
    ...
    var confirmations = "0";
    if(transaction.details.meta.status != undefined && Object.keys(transaction.details.meta.status).includes('Ok'))
        confirmations = "6"
    let from
    let to
    if(out == 1){
        from = account
        var auxSending = transaction.details.transaction.message.accountKeys.filter(a => !a.signer && !accounts.includes(a.pubkey.toString()))
        ...
        to = account
        from = transaction.details.transaction.message.accountKeys.filter(a => a.signer)[0].pubkey.toString()
    }
