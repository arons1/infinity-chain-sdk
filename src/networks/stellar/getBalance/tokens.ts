...
export const getAccountBalances = async ({
    ...
    const reserve = (new BigNumber(10000000)).plus(new BigNumber(accountBalances.subentry_count*0.5).shiftedBy(7)).toString(10)
    ...
    for(let balance of accountBalances.balances){
        ...
        if(balanceRes.asset_type == "native"){
            result[account].push({
                ...
                value:new BigNumber(balanceRes.balance).shiftedBy(7).plus(reserve).toString(10),
                freeze:new BigNumber(balanceRes.balance).shiftedBy(7).toString(10)
            })
        }
        else{
            result[account].push({
                ...
                value:new BigNumber(balanceRes.balance).shiftedBy(7).toString(10)
            })
        }
    }
    return result
