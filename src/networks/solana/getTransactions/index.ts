...
import { AddressesFormat, GetAccountsHashesParams, GetAccountsTransactionsHashesParams, GetAccountsTransactionsParams, GetBatchAddressesWithPaginationParams, GetTransactionsParams, HashesDetails, HashesResult, PaginationData, ResultBatch, ResultSignature } from "./types"
...
export const getAccountsHashes = async ({
    ...
    for(;;){
      ...
      const numberOfHashes = transactionHashes.reduce((p:number,v:HashesResult)=>p + v.hashes.length,0)
      if(numberOfHashes == numberLast)
        break;
...
export const getTransactions = async ({
  ...
  while(pendingTransactions.length > 0){
    for(let i = 0;i<pendingTransactions.length;i += LIMIT_BATCH){
      ...
      const detailsTransactions:ParsedTransactionWithMeta[] = await web3.getParsedTransactions(hashesPending,{maxSupportedTransactionVersion: 0,commitment:"confirmed"})
      transactionsRes = [...transactionsRes,...detailsTransactions.filter(a => a != undefined)]
      await sleep(SLEEP_BETWEEN_CALLS)
    }
    ...
    pendingTransactions = pendingTransactions.filter(a => hashes[a] == undefined)
...
const getBatchAddressesWithPagination = async ({
    ...
    for (var i = 0; i < resultBatchs.length; i++) {
        if (resultBatchs[i].result.length == LIMIT_CALLS) {
            pagination[resultBatchs[i].address] = resultBatchs[i].result[LIMIT_CALLS - 1]
            needRecall = true
        } else {
            if (pagination[resultBatchs[i].address] != undefined)
                delete pagination[resultBatchs[i].address]
            addresses = addresses.filter(a => resultBatchs[i].address != a.address)
        }
    }
    if (needRecall) {
        ...
        for (var i = 0; i < resultBatchsAux.length; i++) {
            for (var j = 0; j < resultBatchs.length; j++) {
                if (resultBatchs[j].address == resultBatchsAux[i].address) {
                    resultBatchs[j].result = [...resultBatchs[j].result, ...resultBatchsAux[i].result]
                    break;
                }
            }
        }
...
const getBatchAddresses = async ({
    ...
    const batch = addresses.map(({
        address
    }) => {
        const data:PaginationData = { limit }
        if(pagination[address])
          data.before = pagination[address]
        if(signatures[address])
          data.until = signatures[address]
