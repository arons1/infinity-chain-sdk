...
class TrezorWebsocket {
    ...
    send(method:string, params:Record<string,any>, callback:(rs?:any) => void) {
      try{
        if(!this.connected)
        {
          callback()
    ...
    subscribe(method:string, params:Record<string,any>, callback: (rs?:any) => void) {
      try{
          if(!this.connected){
            var ts =this;
            this.connect();
          ...
          if(this.ws && this.ws.readyState === this.ws.OPEN){
              ...
              console.log("[ERROR] "+this.coin + " websockets: Trying to subscribe an address with the connection closed")
          }
          return id;
      }
      catch(e){
    ...
    unsubscribe(method:string, id:string, params:Record<string,any>, callback:(rs?:any) => void) {
      try{
        ...
        if(this.ws && this.ws.readyState === this.ws.OPEN){
            ...
            console.log("[ERROR] "+this.coin + " websockets: Trying to unsubscribe an address with the connection closed")
        }
        return id;
      }
      catch(e){
    ...
    subscribeAddresses(wallet:string,addresses:string[],callback:(pr?:any) => void) {
        ...
        this.subscribeAddressesId = this.subscribe('subscribeAddresses', {addresses}, this.callbacks[wallet]);
    }

    unsubscribeAddresses() {
        var _ = this;
        this.unsubscribe('unsubscribeAddresses', this.subscribeAddressesId, {}, () => {
    ...
    connect() {
        ...
        this.ws.onopen = function () {
            ...
            { 
                if(pusher.ws != undefined && pusher.ws.readyState === pusher.ws.OPEN)
        ...
        this.ws.onclose = function () {
            ...
            console.log(pusher.coin + " websocket disconected, reconnecting in 30 seconds")
